...
const OUTCOME_COLORS: Record<string, string> = {
  ...
  FLAGGED_REVIEW: "bg-yellow-100 text-yellow-800",
...
export default function Outcomes() {
  ...
  const handleAddOutcome = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("call_outcomes").insert({
      ...
      child_names: addForm.child_names || null,
      venue_name: addForm.venue_name || null,
      outcome: addForm.outcome,
      transcript: addForm.transcript || null,
      summary: addForm.summary || null,
      attempt_number: parseInt(addForm.attempt_number) || 1,
    });
    ...
    if (error) {
      ...
      // Auto-add to DNC if terminal outcome
      if (addForm.outcome === "BOOKED" || addForm.outcome === "DECLINED") {
        await supabase.from("do_not_call").insert({
          user_id: user.id,
          phone_number: addForm.phone_number,
          reason: addForm.outcome,
          parent_name: addForm.parent_name || null,
          venue_name: addForm.venue_name || null,
        });
      }
  ...
  const filtered = outcomes.filter((o) => {
    ...
    if (searchTerm) {
      ...
      return (
        ...
        (o.child_names || "").toLowerCase().includes(term)
      );
    }
    return true;
  });
    ...
    <DashboardLayout>
      <div className="space-y-6 animate-fade-in">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Call Outcomes</h1>
            <p className="text-muted-foreground mt-1">Master outcomes sheet — all venues, all rounds.</p>
          </div>
            ...
            <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleAddOutcome} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select value={addForm.campaign_id} onValueChange={(v) => {
                    const camp = campaigns.find((c) => c.id === v);
                    setAddForm({ ...addForm, campaign_id: v, venue_name: camp?.venue_name || "" });
                  }}>
                ...
                <div className="grid gap-4 grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input value={addForm.phone_number} onChange={(e) => setAddForm({ ...addForm, phone_number: e.target.value })} required />
                  </div>
                  <div className="space-y-2">
                    <Label>Outcome *</Label>
                    <Select value={addForm.outcome} onValueChange={(v) => setAddForm({ ...addForm, outcome: v })}>
                ...
                <div className="grid gap-4 grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input value={addForm.parent_name} onChange={(e) => setAddForm({ ...addForm, parent_name: e.target.value })} />
                  </div>
                  <div className="space-y-2">
                    <Label>Child Names</Label>
                    <Input value={addForm.child_names} onChange={(e) => setAddForm({ ...addForm, child_names: e.target.value })} />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label>Summary</Label>
                  <Textarea value={addForm.summary} onChange={(e) => setAddForm({ ...addForm, summary: e.target.value })} rows={2} />
                </div>
                <div className="space-y-2">
                  <Label>Transcript</Label>
                  <Textarea value={addForm.transcript} onChange={(e) => setAddForm({ ...addForm, transcript: e.target.value })} rows={3} />
                </div>
        ...
        <div className="flex gap-3 flex-wrap items-end">
          ...
          <Select value={filterOutcome} onValueChange={setFilterOutcome}>
            ...
            <SelectContent>
              {OUTCOMES.map((o) => <SelectItem key={o} value={o}>{o === "ALL" ? "All Outcomes" : o}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        ...
        <Card>
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="p-8 text-center text-muted-foreground">No outcomes found.</div>
            ) : (
              <div className="overflow-auto">
                ...
                {filtered.length > 100 && (
                  <p className="text-xs text-muted-foreground p-3">Showing 100 of {filtered.length}</p>
                )}
              </div>
            )}
          </CardContent>
              ...
              <div className="space-y-4">
                ...
                {selectedOutcome.summary && (
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{selectedOutcome.summary}</p>
                  </div>
                )}
                {selectedOutcome.transcript && (
                  <div>
                    <p className="text-sm font-medium mb-1">Transcript</p>
                    <pre className="text-xs bg-muted p-3 rounded-lg whitespace-pre-wrap max-h-60 overflow-y-auto">{selectedOutcome.transcript}</pre>
                  </div>
                )}
              </div>
            )}
