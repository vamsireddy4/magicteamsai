...
export default function DataCleaning() {
  ...
  const processFile = async () => {
    if (!customerFile) {
      toast({ title: "Missing file", description: "Upload a CSV file first.", variant: "destructive" });
      return;
    }
    setProcessing(true);
    try {
      ...
      toast({ title: "Error analyzing CSV", description: err.message, variant: "destructive" });
    } finally {
      setProcessing(false);
    }
  };
  ...
  // Find the phone column for campaign contacts
  ...
  const startBulkCalling = async () => {
    ...
    if (!phoneCol) {
      toast({ title: "No phone column detected", description: "CSV must have a phone number column.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const contactRows = selected.map((row) => ({
        ...
        first_name: row[nameCol] || row[Object.keys(row)[0]] || "",
        child_names: row.child_names || row.child_name || row.children || null,
        venue_name: row.venue_name || row.venue || null,
        venue_location: row.venue_location || row.location || null,
        start_date: row.start_date || null,
        end_date: row.end_date || null,
        times: row.times || row.time || null,
      ...
      <div className="space-y-6 animate-fade-in">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Upload a CSV → AI analyzes &amp; extracts contacts → select &amp; start bulk calling.
          </p>
        </div>
        ...
        <div className="flex gap-3">
          <Button onClick={processFile} disabled={processing || !customerFile}>
            {processing ? (
              ...
              <><Sparkles className="h-4 w-4 mr-2" /> Process &amp; Analyze</>
            )}
          </Button>
          {rows.length > 0 && (
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>{selectedIndices.size} of {rows.length} selected</CardDescription>
              </div>
            </CardHeader>
            <CardContent>
                  ...
                  <TableBody>
                    {rows.map((row, i) => (
                      <TableRow key={i} className={selectedIndices.has(i) ? "bg-primary/5" : ""}>
                        ...
                        {columns.map((col) => (
                          <TableCell key={col.key} className={col.type === "phone" ? "font-mono text-xs" : "text-sm"}>
                            {row[col.key] || "—"}
                          </TableCell>
                        ))}
                      </TableRow>
        ...
        {rows.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Call {selectedIndices.size} selected contacts using your AI agent.
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="mt-4">
                <Button
                  ...
                  disabled={calling || selectedIndices.size === 0 || !selectedAgent || !selectedPhoneConfig}
                  size="lg"
                >
                  {calling ? (
                    <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Starting Campaign...</>
                  ) : (
                    <><Play className="h-4 w-4 mr-2" /> Call {selectedIndices.size} Contacts</>
                  )}
                </Button>
              </div>
            </CardContent>
